...
public:
    ...
    // 返回值，用来表示tree不经过root的最大路径和
    int gao(TreeNode* root, int &max_across_root) {
        if (root == NULL)
            return 0;
        int left = gao(root->left, max_across_root);
        int right = gao(root->right, max_across_root);
        int cur_max = root->val;
        if (left > 0)
            cur_max += left;
        if (right > 0)
            cur_max += right;
        max_across_root = max(max_across_root, cur_max);
